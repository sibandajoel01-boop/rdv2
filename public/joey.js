...
- Never use ** for bold or * for italic or any markdown formatting in responses
- Write in flowing plain text with no bullet stars or dashes
- Always use commas to connect thoughts, never dashes
- Always score out of 100, never out of 10
...
- Family visa: sponsor spouse and children, requires minimum salary (AED 4,000 with accommodation or AED 3,000 without)
...
BUSINESS SETUP:
- Free zone vs mainland: Free zone gives 100% foreign ownership, restricted to trading within free zone or internationally, mainland allows trading anywhere in UAE
- Popular free zones: DMCC, Dubai Internet City, Dubai Media City, IFZA, Meydan
...
- Freelancer permits from around AED 7,500/year
- Key link: /pillar-business.html

BANKING & FINANCE:
- Major UAE banks: Emirates NBD, FAB, ADCB, Mashreq, RAKBANK, HSBC UAE
...
- UK tax: must pass HMRC Statutory Residence Test to break UK tax residency
...
- Rent (annual): Studio AED 35,000 to 60,000, 1-bed AED 60,000 to 140,000, 2-bed AED 90,000 to 220,000, 3-bed villa AED 150,000 to 350,000
- Popular areas: Dubai Marina, JBR, Downtown Dubai (premium), JLT, Business Bay (mid-range), Jumeirah, Arabian Ranches (family)
- International school fees: AED 40,000 to AED 100,000/year per child
- Utilities (DEWA): AED 500 to AED 2,000/month
- Groceries: AED 2,000 to AED 3,500/month for a family
- Exchange rate: approx 4.65 AED to £1
- Key link: /pillar-cost.html

EDUCATION:
- British curriculum schools widely available and popular with UK expats
- KHDA regulates and rates schools in Dubai: Outstanding, Good, Acceptable, Weak
- Popular schools: GEMS Wellington, JESS, DESS, Nord Anglia
- Apply 6 to 12 months in advance for popular schools
...
Always end responses about a specific topic with a relevant link from the site.
...
5. "Will you need schooling for any children?" (skip if no kids)
6. "Are you planning to rent or are you interested in buying property?"
7. "What is your rough timeline, within 6 months, within a year, or further out?"

Once you have the answers, calculate a Dubai Readiness Score out of 100:
...
Present the score like this: "Based on everything you have told me, your Dubai Readiness Score is [X]/100." Then give 3 to 5 personalised pieces of advice in plain flowing sentences, no stars, no markdown, no bullet dashes. Use commas to connect thoughts. Be warm, honest, and specific. Never be harsh. After the advice always ask: "Would you like me to email you a copy of your full report? Just share your email address and I will send it over straight away." Always finish with the most relevant link.

SALARY DISCLAIMER: whenever you give salary figures, always add: "Please keep in mind that salary figures are estimates only, as actual offers vary widely depending on the company, your experience, and the role."
...
function showEmailCapture(score, summary, nextSteps) {
  ...
  div.style.cssText = 'background:#fff;border:1.5px solid #C9913A;border-radius:12px;padding:16px;margin-top:8px;max-width:82%;';
  div.innerHTML = `
    ...
    <p style="margin:0 0 12px;font-size:0.82rem;color:#5A6A7A;line-height:1.5;">Enter your email and I will send you a personalised report with your score, summary and next steps.</p>
...
function injectStyles() {
  ...
  style.textContent = `
    #joey-bubble { position:fixed; bottom:28px; right:28px; z-index:9999; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; }
    #joey-toggle { height:52px; padding:0 20px 0 14px; border-radius:30px; background:#0B1F3A; border:2px solid #C9913A; cursor:pointer; display:flex; align-items:center; gap:10px; box-shadow:0 4px 24px rgba(11,31,58,0.45); transition:transform 0.2s ease,box-shadow 0.2s ease; margin-left:auto; position:relative; white-space:nowrap; }
    #joey-toggle:hover { transform:translateY(-2px); box-shadow:0 8px 32px rgba(11,31,58,0.55); }
    .joey-toggle-avatar { width:30px; height:30px; border-radius:50%; background:#C9913A; display:flex; align-items:center; justify-content:center; font-family:Georgia,serif; font-size:0.95rem; font-weight:700; color:#0B1F3A; flex-shrink:0; }
    .joey-toggle-text { display:flex; flex-direction:column; align-items:flex-start; }
    .joey-toggle-name { color:#fff; font-size:0.85rem; font-weight:600; line-height:1.2; }
    .joey-toggle-sub { color:#C9913A; font-size:0.7rem; line-height:1.2; }
    #joey-toggle .joey-close-icon { display:none; color:#C9913A; font-size:1.2rem; line-height:1; margin:0 auto; }
    #joey-bubble.open #joey-toggle .joey-toggle-avatar, #joey-bubble.open #joey-toggle .joey-toggle-text { display:none; }
    #joey-bubble.open #joey-toggle .joey-close-icon { display:block; }
    #joey-bubble.open #joey-toggle { padding:0 18px; justify-content:center; }
    #joey-notification { position:absolute; top:-4px; right:-4px; width:16px; height:16px; background:#C9913A; border-radius:50%; border:2px solid #0B1F3A; display:none; }
    #joey-notification.show { display:block; }
    #joey-window { position:absolute; bottom:66px; right:0; width:360px; max-height:520px; background:#fff; border-radius:16px; box-shadow:0 12px 48px rgba(11,31,58,0.22); display:flex; flex-direction:column; overflow:hidden; opacity:0; transform:translateY(12px) scale(0.97); pointer-events:none; transition:opacity 0.25s ease,transform 0.25s ease; }
    #joey-bubble.open #joey-window { opacity:1; transform:translateY(0) scale(1); pointer-events:all; }
    #joey-header { background:#0B1F3A; padding:14px 16px; display:flex; align-items:center; gap:12px; flex-shrink:0; }
    .joey-header-avatar { width:38px; height:38px; border-radius:50%; background:#C9913A; display:flex; align-items:center; justify-content:center; font-family:Georgia,serif; font-size:1.1rem; font-weight:700; color:#0B1F3A; flex-shrink:0; }
    .joey-header-info { flex:1; }
    .joey-header-name { color:#fff; font-weight:600; font-size:0.95rem; line-height:1.2; }
    .joey-header-status { color:rgba(255,255,255,0.55); font-size:0.75rem; display:flex; align-items:center; gap:5px; }
    .joey-status-dot { width:7px; height:7px; border-radius:50%; background:#4CAF50; display:inline-block; }
    #joey-messages { flex:1; overflow-y:auto; padding:16px; display:flex; flex-direction:column; gap:12px; background:#F7F8FA; }
    #joey-messages::-webkit-scrollbar { width:4px; }
    #joey-messages::-webkit-scrollbar-thumb { background:#ddd; border-radius:4px; }
    .joey-msg { max-width:82%; padding:10px 14px; border-radius:14px; font-size:0.875rem; line-height:1.55; word-break:break-word; }
    .joey-msg a { color:#C9913A; text-decoration:underline; font-weight:500; }
    .joey-msg.joey { background:#fff; color:#1A202C; border-bottom-left-radius:4px; align-self:flex-start; box-shadow:0 1px 4px rgba(0,0,0,0.07); }
    .joey-msg.user { background:#0B1F3A; color:#fff; border-bottom-right-radius:4px; align-self:flex-end; }
    .joey-typing { display:flex; align-items:center; gap:5px; padding:10px 14px; background:#fff; border-radius:14px; border-bottom-left-radius:4px; align-self:flex-start; box-shadow:0 1px 4px rgba(0,0,0,0.07); width:fit-content; }
    .joey-typing span { width:7px; height:7px; border-radius:50%; background:#C9913A; animation:joeyBounce 1.2s infinite; }
    .joey-typing span:nth-child(2) { animation-delay:0.2s; }
    .joey-typing span:nth-child(3) { animation-delay:0.4s; }
    @keyframes joeyBounce { 0%,60%,100% { transform:translateY(0); opacity:0.4; } 30% { transform:translateY(-5px); opacity:1; } }
    #joey-input-area { padding:12px; background:#fff; border-top:1px solid #E2E8F0; display:flex; gap:8px; align-items:flex-end; flex-shrink:0; }
    #joey-input { flex:1; border:1.5px solid #E2E8F0; border-radius:10px; padding:9px 12px; font-size:0.875rem; font-family:inherit; outline:none; resize:none; max-height:90px; line-height:1.4; color:#1A202C; transition:border-color 0.2s; }
    #joey-input:focus { border-color:#C9913A; }
    #joey-send { width:38px; height:38px; border-radius:10px; background:#0B1F3A; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:background 0.2s,transform 0.15s; }
    ...
    .joey-suggestions { display:flex; flex-wrap:wrap; gap:6px; padding:8px 16px 4px; background:#F7F8FA; }
    .joey-suggestion { background:#fff; border:1px solid #C9913A; color:#0B1F3A; font-size:0.75rem; padding:5px 10px; border-radius:20px; cursor:pointer; transition:background 0.15s,color 0.15s; font-family:inherit; }
    .joey-suggestion:hover { background:#C9913A; color:#fff; }
    @media (max-width:480px) { #joey-window { width:calc(100vw - 32px); right:0; bottom:76px; } #joey-bubble { bottom:20px; right:16px; } }
  `;
    ...
    <div id="joey-window">
      <div id="joey-header">
        ...
        <div class="joey-header-info">
          ...
          <div class="joey-header-status"><span class="joey-status-dot"></span> Dubai Relocation Guide</div>
        </div>
      </div>
...
function toggleJoey() {
  ...
  if (isOpen) {
    setTimeout(() => { document.getElementById('joey-input').focus(); scrollToBottom(); }, 280);
  }
}
...
async function handleSend() {
  ...
  try {
    ...
    if (detectedScore !== null) {
      setTimeout(() => showEmailCapture(detectedScore, extractSummary(reply), extractNextSteps(reply)), 600);
    }
...
function addMessage(role, text) { renderMessage(role, text); scrollToBottom(); }
...
function saveMessages() {
  try { sessionStorage.setItem('joey_messages', JSON.stringify(messages.slice(-20))); } catch(e) {}
}
